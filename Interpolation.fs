...
let updateLagrangePoints (currentPoints: seq<float * float>) (newPoint: float * float) =
    if Seq.length currentPoints >= 4 then
...
let updateLinearPoints (currentPoints: seq<float * float>) (newPoint: float * float) =
    Seq.append (Seq.skip 1 currentPoints) (Seq.singleton newPoint)
